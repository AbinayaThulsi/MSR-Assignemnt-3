...
from metric import cmAccuracy, cmPrecision, cmRecall
from ranking import RecallRate, RankingResultFile, generateRecommendationList, SunRanking
...
def logMetrics(expRun, logger, metrics, epoch):
    for metricName, metricValue in metrics.items():
        ...
        if isinstance(metricValue, dict):
            ...
            logger.info(
                {'type': 'metric', 'label': metricName, 'value': metricValue, 'epoch': epoch})

        expRun.log_scalar(metricName, metricValue, step=epoch)


def logRankingResult(expRun, logger, rankingClass, rankingScorer, bugReportDatabase, rankingResultFilePath, epoch,
    ...
    logger.info(
        '[{}] Throughput: {} bugs per second (bugs={} ,seconds={})'.format(label, (nDupBugs / duration), nDupBugs,
                                                                           duration))
