...
def undoSort(x, lengths, sortedIdxs):
    ...
    """''
    # Old version
    # originalIdx = Variable(torch.zeros(sortedIdxs.size()).long().scatter_(0, sortedIdxs.cpu().data,
    #                                                                       torch.arange(0, len(lengths)).long()))
    # if cudaOn:
    #     originalIdx = originalIdx.cuda()
    # k= torch.index_select(x, dim=0, index=originalIdx)
...
def createEmbeddingLayer(embeddingObject, updateEmbedding):
    embedding = nn.Embedding(embeddingObject.getNumberOfVectors(), embeddingObject.getEmbeddingSize(),
...
class RNNEncoder(nn.Module):
    ...
    def __init__(self, rnnType, embeddingObject, hiddenSize=100, numLayers=1, bidirectional=False, updateEmbedding=True,
                 dropout=0, isLengthVariable=True):
        ...
        if rnnType == 'lstm':
            self.rnnEncoder = nn.LSTM(inputSize, self.hiddenSize, self.numLayers,
                                      batch_first=True, bidirectional=bidirectional, dropout=self.dropout)
        elif rnnType == 'gru':
            self.rnnEncoder = nn.GRU(inputSize, self.hiddenSize, self.numLayers,
                                     batch_first=True, bidirectional=bidirectional, dropout=self.dropout)
        else:
...
class SortedRNNEncoder(nn.Module):
    ...
    def __init__(self, rnnType, embeddingObject, hiddenSize=100, numLayers=1, bidirectional=False, updateEmbedding=True,
        ...
        self.rnnEncoder = RNNEncoder(rnnType, embeddingObject, hiddenSize, numLayers, bidirectional,
                                     updateEmbedding, dropout, True)
...
class TextCNN(nn.Module):
    ...
    def __init__(self, windowSizes, nFilters, wordEmbedding, updateEmbedding, activationFunc=F.relu,
                 batchNormalization=False, dropout=0.0):
        super(TextCNN, self).__init__()

        embeddingSize = wordEmbedding.getEmbeddingSize()
        self.embedding = nn.Embedding(wordEmbedding.getNumberOfVectors(), embeddingSize,
...
class MultilayerDense(Module):
    ...
    def __init__(self, previousLayer, hiddenSizes, activationFunc, batchNormalization=False, applyBatchLastLayer=True,
                 dropoutLastLayer=0.0, layerNorm=False):
