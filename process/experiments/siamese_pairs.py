...
from collate import PairBugCollate
from dataset import PairBugDatasetReader
from preprocessing import PreprocessorList
from offline_pair_geneneration import NonNegativeRandomGenerator, PreSelectedGenerator, \
    RandomGenerator, KRandomGenerator, MiscNonZeroRandomGen, MiscOfflineGenerator
from metric import AverageLoss, MeanScoreDistance, ConfusionMatrix, cmAccuracy, cmPrecision, cmRecall, \
    PredictionCache, LossWrapper
from ranking import PreselectListRanking, SharedEncoderNNScorer, \
    SunRanking, DeshmukhRanking, GeneralScorer
from loss import CosineLoss, NeculoiuLoss
from siamese import ProbabilityPairNN, CosinePairNN
from jsontools import JsonLogFormatter
from siamese_util import processSumDescParam, processSumParam, processCategoricalParam, processDescriptionParam
from training_loop_util import logMetrics, logRankingResult, logConfusionMatrix

ex = Experiment("siamese_pair")
...
def main(_run, _config, _seed, _log):
    ...
    if args.get('pairs_training'):
        ...
        if not offlineGeneration:
            ...
            pairTrainingReader = PairBugDatasetReader(pairsTrainingFile, preprocessors,
            ...
            if pairGenType == 'random':
                ...
                negativePairGenerator = RandomGenerator(preprocessors, pairCollate,
                ...
                negativePairGenerator = NonNegativeRandomGenerator(preprocessors, pairCollate,
                                                                   negativePairGenOpt['rate'],
                ...
                negativePairGenerator = MiscNonZeroRandomGen(preprocessors, pairCollate,
                                                             negativePairGenOpt['rate'], bugIds,
                                                             trainingDataset.duplicateIds, masterIdByBugId, device,
                ...
                negativePairGenerator = KRandomGenerator(preprocessors, pairCollate,
                                                         negativePairGenOpt['rate'],
                ...
                negativePairGenerator2 = NonNegativeRandomGenerator(preprocessors, pairCollate,
                                                                    negativePairGenOpt['rate'],
            ...
            pairTrainingReader = PairBugDatasetReader(pairsTrainingFile, preprocessors, negativePairGenerator,
                                                      randomInvertPair=args['random_switch'])

        trainingLoader = DataLoader(pairTrainingReader, batch_size=batchSize, collate_fn=pairCollate.collate,
                                    shuffle=True)
    ...
    trainingMetrics = {'training_loss': AverageLoss(lossFn),
                       'training_dist_target': MeanScoreDistance(negTarget=negTarget,
    ...
    validationMetrics = {'validation_loss': LossWrapper(lossFn),
                         'validation_dist_target': MeanScoreDistance(negTarget=negTarget,
                                                                     output_transform=scoreDistanceTrans),
                         'validation_confusion_matrix': ConfusionMatrix(2), }
    ...
    def onEndEpoch(engine):
        ...
        logConfusionMatrix(_run, logger, 'training_confusion_matrix', engine.state.metrics['training_confusion_matrix'],
                           epoch)
        ...
        if validationLoader:
            ...
            logConfusionMatrix(_run, logger, 'validation_confusion_matrix',
    ...
    if pair_test_dataset is not None and len(pair_test_dataset) > 0:
        ...
        testMetrics = {'test_accuracy': ignite.metrics.Accuracy(),
        ...
        for metricName, metricValue in test_evaluator.state.metrics.items():
            ...
            if isinstance(metric, ignite.metrics.Accuracy):
                ...
                logger.info({'type': 'metric', 'label': metricName, 'value': np.float(metricValue.cpu().numpy()[1]),
                             'epoch': None,
